...
export default function ErrorDetails({ selectedError }) {
  ...
  const getNormalizedStack = () => {
    if (typeof selectedError.rawStack === "string")
      return selectedError.rawStack;
    if (typeof selectedError.stack === "string") return selectedError.stack;
    if (
      Array.isArray(selectedError.stackTrace) &&
      selectedError.stackTrace.length > 0
    ) {
  ...
  const handleCopyRawJson = () => {
    const rawJsonString = JSON.stringify(selectedError, null, 2);
  ...
  const handleAskGemini = async () => {
    ...
    try {
      ...
      // Note: Make sure to set VITE_GEMINI_API_KEY in your .env file
      const ai = new GoogleGenAI({
        apiKey: import.meta.env.VITE_GEMINI_API_KEY || "",
        dangerouslyAllowBrowser: true,
      });

      const prompt = `Analyze this application error payload, explain the root cause clearly, and suggest precise code fixes or troubleshooting steps:\n\n${JSON.stringify(selectedError, null, 2)}`;
      ...
      setAiAnalysis(
        "Failed to generate analysis. Please check if your API key is correctly configured in your environment variables.",
      );
    } finally {
  ...
  return (
    <div className="relative lg:col-span-7 bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col gap-6 text-slate-100 overflow-hidden">
      {/* Incident Header */}
      <div className="flex items-start justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span
              className={`px-2 py-0.5 rounded text-xs font-mono border font-semibold ${getStatusColor(selectedError.statusCode)}`}
            >
      ...
      <div className="bg-slate-950/70 border border-slate-800/80 rounded-lg p-3.5 space-y-2 text-xs font-mono">
        <div className="flex items-center gap-2 text-slate-300">
          ...
          <span className="text-indigo-300 font-semibold">
            {selectedError.origin?.endpoint || "N/A"}
          </span>
        </div>
        <div className="flex items-center gap-2 text-slate-300">
          ...
          <span className="text-slate-300 truncate">
            {selectedError.origin?.filePath || "N/A"}
          </span>
        </div>
      </div>
                ...
                >
                  <span
                    className={`absolute -left-[7px] top-1 w-3 h-3 rounded-full border-2 border-slate-900 ${
                      crumb.level === "error" || crumb.isError
                        ? "bg-red-500"
                        : "bg-indigo-500"
                    }`}
                  />
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span
                            className={`px-1.5 py-0.5 rounded text-[10px] font-mono border uppercase font-medium ${getLevelBadge(crumb.level, crumb.isError)}`}
                          >
                    ...
                    {hasPayload && (
                      <div className="pt-1 border-t border-slate-800/60">
                        ...
                        >
                          ...
                          <span>
                            Payload Context ({Object.keys(crumb.data).length}{" "}
                            keys)
                          </span>
                        </button>
      ...
      {activeTab === "stack" && (
        <div className="relative group">
          {stackString ? (
            <pre className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-[11px] font-mono text-red-300/90 overflow-x-auto whitespace-pre-wrap max-h-[380px] leading-relaxed scrollbar-thin">
              {stackString}
            </pre>
          ) : (
        ...
        <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1 scrollbar-thin">
          ...
          <div>
            ...
            <pre className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-[11px] font-mono text-amber-300/90 overflow-x-auto whitespace-pre-wrap">
              {selectedError.errorRaw
                ? JSON.stringify(selectedError.errorRaw, null, 2)
                : "null"}
            </pre>
          </div>
        </div>
      ...
      {/* Tab 4: Full Error JSON */}
      {activeTab === "json" && (
        <div className="relative group space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-400">
              Complete API Error Payload
            </span>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleAskGemini}
                className="px-2.5 py-1 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 rounded border border-indigo-500/30 transition-colors text-xs flex items-center gap-1.5 cursor-pointer font-mono"
              >
              ...
              <button
                ...
                className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded border border-slate-700 transition-colors text-xs flex items-center gap-1.5 cursor-pointer font-mono"
              >
                {copiedRawJson ? (
          ...
          <pre className="bg-slate-950 p-4 rounded-lg border border-slate-800 text-[11px] font-mono text-emerald-300/90 overflow-x-auto whitespace-pre-wrap max-h-[380px] leading-relaxed scrollbar-thin">
            {JSON.stringify(selectedError, null, 2)}
          </pre>
        </div>
      )}
        ...
        <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-sm z-50 p-6 flex flex-col gap-4 animate-fadeIn">
            ...
            {isAnalyzing ? (
              <div className="h-full flex flex-col items-center justify-center gap-3 text-slate-400 py-16">
                ...
                <p className="text-xs font-mono">
                  Analyzing error context and trace patterns...
                </p>
              </div>
            ) : (
          ...
          <div className="pt-3 border-t border-slate-800 flex justify-end">
            <button
              type="button"
              onClick={() => setShowAiModal(false)}
              className="px-4 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-mono rounded border border-slate-700 transition-colors cursor-pointer"
            >
              Close Analysis
            </button>
          </div>
